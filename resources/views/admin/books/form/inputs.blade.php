...
{{--                            {{$category->title == $book->cat_name ? 'selected' : ''}}--}}
                ...
                <div class="col s12 input-field">
                        <input type="text" name="title" id="title" class="@error('title') invalid @enderror" value="{{$book->title??old('title')}}">
        ...
        <div class="col s12 input-field">
            ...
            <li>
                ...
                <div class="collapsible-body"><span>
                        <textarea name="detail" id="detail" cols="30" rows="10">{{$book->detail??old('detail')}}</textarea>
                    </span></div>
            </li>
        </ul>
        </div>
...
<div class="container">
    <div class="file-field input-field">
        <div class="btn">
            <span>Image</span>
            <input type="file" accept="image/*" name="photo" value="{{isset($book)?asset('storage/'.$book->photo):old('photo')}}">
        </div>
        <div class="file-path-wrapper @error('photo') invalid @enderror">
            <input class="file-path validate" name="photo" id="photo" accept="image/*"
                   ...
                   value="{{isset($book)?asset('storage/'.$book->photo):old('photo')}}">
        </div>
        @error('photo')
        <blockquote>{{$message}}</blockquote>
        @enderror
...
{{--                {{isset($book)?asset('storage/'.$book->file):''}}--}}
            ...
            <div class="file-path-wrapper @error('file') invalid @enderror">
                <input class="file-path validate" name="file" id="file" placeholder="Upload File" value="{{isset($book)?asset('storage/'.$book->file):old('file')}}">
            </div>
            @error('file')
            <blockquote>{{$message}}</blockquote>
            @enderror
